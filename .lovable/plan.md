...
4. **Boucle séquentielle, une entrée à la fois** (jamais de parallélisme : le service ne rend qu'une page à la fois) :
   ...
   - b. **Appel du rendu** : `POST {PRERENDER_SERVICE_URL}` avec `Authorization: Bearer {PRERENDER_SERVICE_TOKEN}`, corps `{ url, wait_for: "window.__PRERENDER_READY__ === true", timeout_ms: 30000 }` — au-delà du plafond interne de 20 s de la page. L'URL absolue est reconstruite depuis le site public + `url_path`.
