...
function CategoryCard({ cat }: { cat: CategoryData }) {
  return (
    <Link
      to={`/recherche?categorie=${encodeURIComponent(cat.slug)}`}
    ...
    <div className="bg-card rounded-md shadow-card overflow-hidden flex flex-col">
      ...
      <div className="relative h-52 md:h-64 bg-secondary/30 flex items-center justify-center shrink-0">
        ...
        {provider.est_premium && (
          <span className="absolute top-4 right-4 bg-or-riche text-white text-xs uppercase tracking-wider px-3 py-1.5 rounded-md font-sans font-medium">
            Premium
          </span>
        )}
      </div>
