...
export default function HeaderMegaMenuPrestataires({ onNavigate }: Props) {
  ...
  return (
    <div
      className="absolute left-0 right-0 top-full bg-[hsl(var(--header-ivoire))] shadow-[var(--shadow-header-mega)] border-t border-[hsl(var(--header-or-fonce)/0.15)] z-40 animate-in fade-in slide-in-from-top-1 duration-150 max-h-[calc(100vh-6rem-1rem)] overflow-y-auto"
    >
        ...
        {isLoading || !familles ? (
          ...
          <>
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-10 gap-y-6">
              {familles.map((f) => (
                <div key={f.cle}>
                  <h4 className="flex items-center gap-3 text-[10.5px] font-sans font-semibold uppercase tracking-[0.2em] text-[hsl(var(--header-or-fonce))] mb-2">
                    <span>{f.libelle}</span>
                    ...
                    {f.meres.map((m) => (
                      <li key={m.id}>
                        <Link
                          ...
                          onClick={onNavigate}
                          className="group flex items-center gap-3 -mx-2 px-2 py-1 rounded-md hover:bg-[hsl(var(--header-or-fonce)/0.06)] transition-colors"
                        >
                        ...
                        {m.enfants.length > 0 && (
                          <ul className="ml-[54px] mt-0.5 mb-0.5 space-y-0">
                            {m.enfants.map((s) => (
                              <li key={s.id}>
                                <Link
                                  to={`/prestataires/${m.slug}/${s.slug}`}
                                  onClick={onNavigate}
                                  className="block py-0.5 font-sans text-[12.5px] text-muted-foreground hover:text-[hsl(var(--header-or-fonce))] transition-colors"
                                >
                                  · {s.nom}
                                </Link>
                              </li>
