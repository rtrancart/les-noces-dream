...
export interface CharteSignatureFlowProps {
   ...
   *  "resignature" : presta existant signant une nouvelle version. */
  mode: "inscription" | "resignature";
  /** Appelé après signature réussie. Doit gérer la suite (update profile, redirection). */
  onSigned: () => Promise<void> | void;
  /** Permet ou non au presta de reporter la signature (uniquement en resignature). */
  allowReportLater?: boolean;
}

export function CharteSignatureFlow({ mode, onSigned, allowReportLater = false }: CharteSignatureFlowProps) {
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      if (data?.already_signed) {
        toast.info("Vous avez déjà signé la Charte.");
      } else {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background px-4 py-12">
        <Card className="w-full max-w-lg p-10 text-center space-y-5">
          ...
          <Button onClick={() => navigate(mode === "inscription" ? "/espace-pro?welcome=1" : "/espace-pro")} className="mt-4">
          ...
          <Card className="p-6 md:p-10 space-y-6">
            ...
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: articles[step].html.replace(/^\s*<h3[^>]*>[\s\S]*?<\/h3>/i, ""),
              }}
            />
