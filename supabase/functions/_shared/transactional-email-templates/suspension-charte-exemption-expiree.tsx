...
const Email = ({ prenom, nom_commercial, lien_signature }: Props) => (
  <Html lang="fr" dir="ltr">
    <Head />
    <Preview>Votre fiche {SITE_NAME} a été suspendue — signez la Charte pour la réactiver</Preview>
    <Body style={main}>
      <Container style={container}>
        ...
        <Text style={text}>
          Votre fiche <strong>{nom_commercial ?? ''}</strong> bénéficiait jusqu'ici d'une exemption temporaire
          de signature de la Charte Qualité {SITE_NAME}. Cette exemption est arrivée à échéance.
        </Text>
        <Text style={textHighlight}>
          Votre fiche a été suspendue et n'est plus visible du public.
        </Text>
        <Text style={text}>
          Pour rétablir immédiatement sa visibilité, il vous suffit de signer la Charte Qualité depuis votre espace.
          La republication est automatique dès la signature.
        </Text>
        {lien_signature && (
          <Section style={{ textAlign: 'center', margin: '32px 0' }}>
            <Button href={lien_signature} style={button}>Signer la Charte maintenant</Button>
          </Section>
        )}
        <Text style={footer}>À bientôt sur {SITE_NAME}.</Text>
      </Container>
    </Body>
  </Html>
)
...
export const template = {
  component: Email,
  subject: `Votre fiche ${SITE_NAME} a été suspendue — signez la Charte pour la réactiver`,
  displayName: 'Suspension — exemption de charte expirée',
  previewData: {
    prenom: 'Marie',
    nom_commercial: 'Atelier Marie Fleurs',
    lien_signature: 'https://lesnoces.net/signer-la-charte',
  },
