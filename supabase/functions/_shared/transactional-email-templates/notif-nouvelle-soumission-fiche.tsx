...
interface Props {
  ...
  categorie?: string
  ville?: string
  lien_back_office?: string
}

const Email = ({ nom_commercial, categorie, ville, lien_back_office }: Props) => (
  <Html lang="fr" dir="ltr">
    <Head />
    <Preview>Nouvelle fiche prestataire à valider</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={header}>
          <Img src={LOGO_URL} alt={SITE_NAME} width="160" height="48" style={logoImg} />
        </Section>
        <Heading style={h1}>Nouvelle fiche à valider</Heading>
        <Text style={text}>
          <strong>{nom_commercial ?? 'Un prestataire'}</strong>
          {categorie ? ` (${categorie})` : ''}
          {ville ? ` — ${ville}` : ''} vient de soumettre sa fiche pour validation.
        </Text>
        <Text style={text}>
          L'équipe éditoriale doit relire la fiche avant publication. Délai cible : 48h.
        </Text>
        {lien_back_office && (
          <Section style={{ textAlign: 'center', margin: '32px 0' }}>
            <Button href={lien_back_office} style={button}>Ouvrir le back-office</Button>
          </Section>
        )}
        <Text style={footer}>Notification automatique adressée à l'équipe d'administration.</Text>
      </Container>
    </Body>
  </Html>
)
...
export const template = {
  component: Email,
  subject: `Nouvelle fiche à valider — ${SITE_NAME}`,
  displayName: 'Notif nouvelle soumission de fiche',
  previewData: { nom_commercial: 'Atelier Marie Fleurs', categorie: 'Fleuriste', ville: 'Paris', lien_back_office: 'https://lesnoces.net/admin/prestataires' },
