CREATE OR REPLACE FUNCTION public.admin_stats_zones_categories()
 RETURNS TABLE(scope text, zone_value text, zone_type text, label text, parent_region_zone_value text, dept_code text, categorie_mere_id uuid, categorie_nom text, statut statut_prestataire, nb bigint)
 LANGUAGE sql
...
AS $function$
  WITH base AS (
    ...
    WHERE (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'super_admin'))
      AND p.statut NOT IN ('archive', 'suspendu')
  ),
  deplie AS (
